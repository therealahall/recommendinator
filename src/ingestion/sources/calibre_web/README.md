...
- **Series**: read from the schema.org `<schema:Series schema:name="..." schema:position="..."/>` element Calibre-Web emits (position may also appear as a `<schema:position>` child). Bare `<series>` / `<series_index>` children are read as a fallback for non-standard feeds. A position with no series name is dropped, and the index is your catalogue's own: it ranks above a marker in a title, and one you fix in Calibre lands on the next sync.
