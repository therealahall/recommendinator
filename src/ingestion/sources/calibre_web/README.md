...
- **Rating**: Calibre stores ratings as 0–10 in steps of 2 (= 0–5 stars). A rating is only derived from a genuine ratings signal — a `<rating>` element, or a `<category>` whose `scheme` identifies it as a rating. The native 0–10 scale is converted to 0–5 stars and clamped to 1–5; a rating of 0 or absent becomes no rating. Bare numeric category labels with no rating scheme (e.g. a publication year like `2008`) are never mistaken for a rating — they are kept as tags.
