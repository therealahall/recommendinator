...
**Changing `url` or `verify_ssl` clears the stored password**, so it is never
sent to a host, or over a connection, it was not entrusted to. Re-run
`source set-secret` (or use **Replace**) after either edit.
