...
- **Watched history** is imported as completion state:
  ...
  - Shows become `completed` when every aired episode has been watched, otherwise `currently_consuming`. Season progress is tracked in metadata via `seasons_watched` (the **fully-watched** season numbers) and `total_seasons`. Season 0 (specials) is excluded.
  - Each fully-watched season is also dated in `seasons_watched_dates`, a `{season_number: iso_timestamp}` map keyed by the same season numbers as `seasons_watched`. The date is the latest `last_watched_at` among that season's episodes.
- **Completion dates are the viewer's calendar day, not UTC's.** Trakt records watch times as UTC instants. The plugin converts the instant to the machine's timezone before narrowing it to `date_completed`. There is nothing to configure — the zone comes from the host, which under Docker means whatever `TZ` you set on the service (see [DOCKER.md](../../../../docs/DOCKER.md#environment-variables)). Set the zone before the import that matters: a re-sync repairs a stored date only when the new zone is east of the old.
