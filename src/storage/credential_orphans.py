...
def warn_about_orphaned_credentials(
    ...
    """Tell the operator to reconnect *source_id* when its token is stranded.

    Silent once the source holds the key itself: the leftover copy is then
    unreadable rather than the reason a sync fails.
    """
    if source_id == plugin_name:
        return

    stranded = sorted(
        _credential_keys(storage, user_id, plugin_name)
        - _credential_keys(storage, user_id, source_id)
    )
    if not stranded:
        return

    logger.warning(
        "Source '%s' stores no %s of its own: one is filed under the plugin "
        "name '%s' by a release that stored credentials per plugin, and it is "
        "left there because a plugin's sources cannot be told apart. Reconnect "
        "'%s' to store it where this source reads it.",
...
def delete_orphaned_credentials(
    ...
    """
    remaining = configured_source_plugins(config, storage, user_id)
    # Two sources sharing the plugin keep the row: this delete was asked about
    # one of them, and nothing records which rotated that token. A source NAMED
    # after the plugin reads the row as its own, stranded or not.
    if plugin_name in remaining or plugin_name in remaining.values():
        return
