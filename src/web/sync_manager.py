...
# Fixed text for an import that failed before recording any per-item error.
# ``error_message`` is serialised by ``SyncJob.to_dict`` and polled by the
# browser, and a ``FileImportError`` message names the server-side temp file, so
# the exception's own text never goes here.
IMPORT_INTERNAL_FAILURE_MESSAGE = "Import failed due to an internal error"
        ...
        try:
            ...
            with self._lock:
                ...
                if job.error_message is None:
                    # ``error_message`` is serialised to a polling client, and an
                    # import exception's text names the server-side temp file and
                    # forwards raw parser output. Per-item errors are already
                    # written for display, so they may be surfaced; anything else
                    # stays generic and the diagnostic goes to the log only —
                    # same rule ``_run_sync`` follows.
                    job.error_message = (
                        job.errors[0] if job.errors else IMPORT_INTERNAL_FAILURE_MESSAGE
                    )
            # Not ``exc_info=True``: the rendered traceback ends in the
            # exception's own text, and a ``FileImportError`` message can embed
            # a caller-supplied multipart field name (the unknown-option
            # detail). Nothing can sanitise what ``exc_info`` renders, so the
            # message is logged explicitly instead (CWE-117).
            logger.error(
