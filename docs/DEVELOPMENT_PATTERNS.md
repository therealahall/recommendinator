...
4. **For test changes, read `tests/<area>/conftest.py` first.** If a shared fixture or helper exists (`cli_runner`, `_invoke_with_mocks`, `_cli_patches`), use it. Never redefine a fixture locally that already lives in conftest. The repository-root `conftest.py` already does three things for every test in every tree — plugin-local tests under `src/` included — so no test needs to arrange them for itself: it redirects the credential key, neutralises production logging, and pins the process timezone to UTC. A test that needs another zone requests the `host_timezone` fixture and calls it with the zone it wants, which is restored afterwards either way.
...
- **The same guard bans the APIs that build a home-relative path**, so a hostile-input test can trip on its own payload. Exempt that single line with a comment marker rather than changing the payload — the reason is required, and the marker only counts when it opens a comment (`#`, `//`, or `<!--`) — one per language in scope: `#` for Python, YAML and shell, `//` for TypeScript, `<!--` for Markdown — so quoting it in prose exempts nothing. **A security test is never weakened to satisfy the guard.**
...
- **Run `python3.11 -m black src/ tests/ scripts/ conftest.py` and `python3.11 -m ruff check src/ tests/ scripts/ conftest.py --fix` BEFORE `command make check`**, so make check doesn't fail on auto-fixable formatting. The directory list must match what `make check` inspects, or an edit gets format-checked without ever being pre-formatted. See `feedback_always_format_before_check.md`.
...
- **Live regions are mounted persistently; only their text changes.** A region carrying `aria-live` / `role="status"` / `role="alert"` must already be in the DOM, and in the accessibility tree, before it has anything to say. Inserting it with `v-if` at the moment it first has content is the bug this repository has now hit in three separate components: some screen readers (JAWS in particular) treat a region that appears already populated as page content rather than a status change, and skip the announcement entirely (WCAG 4.1.3 Status Messages). Render the region unconditionally — `sr-only` if it should not be seen — and bind its text to a computed that is `''` when there is nothing to announce. **`v-show` is not a substitute:** it applies `display: none`, which removes the node from the accessibility tree, so a `v-show`-gated region has the same first-appearance problem in a different wrapper. Do not double up either — a visible notice and an `sr-only` live region carrying the same words announce twice, so only one of the two gets the live role.
