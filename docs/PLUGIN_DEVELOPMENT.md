...
returns your message now — a write answers with the field it blames, a sync with
a fixed string — but both log it, and redaction there is exact-substring. Return
one error per field, since the field name is all the caller sees.
...
`tests/test_source_paths.py` catches a missing flag only when `path`, `dir`,
`directory`, `file`, `folder` or a plural is a whole underscore-separated part
of the name — `csv_path` yes, `filepath` no. Anything else is on you. The flag
...
```python
ContentItem(
    id="unique-external-id",             # required, unique per source and type
...
Confirm discovery with `uv run python -m src.cli source plugins`, which lists
your plugin.
...
`execute_sync` asks the same method for the id a rotated credential is stored
under, so attribution and ownership cannot drift apart.

**Source identity is framework owned.** `get_source_identifier` and
`transform_config` are `@final`; overriding either raises `TypeError` at class
creation, because a plugin that renamed its source could overwrite another's
credentials. Reshape your own fields in `transform_fields` instead.
...
- **Storage writes are already serialised.** `StorageManager` locks around
  `save_content_item`, `complete_content_item`, `credentials.save` and
