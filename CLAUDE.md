...
conftest.py           # Three autouse fixtures, function-scoped, applying to every test in
                      # every tree: each test is kept off the real credential key and the
                      # production log file, and runs with the process timezone pinned to
                      # UTC (request `host_timezone` and call it to exercise another zone)
