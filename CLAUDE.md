...
├── web/              # FastAPI app: routing, guards, app state. api/ holds one
│                     # module per capability, same shape as cli/commands/, each
│                     # owning a bare router its __init__ mounts on the one
│                     # guarded parent; scheduler.py runs the per-source cadences
│                     # on the lifespan; sync_dispatch.py builds the job it and
│                     # the sync endpoint both start; themes.py the installed-theme
│                     # scan both interfaces read
│   └── static/
...
├── covers/           # Cover fetch guards and the disk cache an enrichment run fills
...
│                     # interfaces read, watch and stop, ui_settings.py
│                     # for the per-user UI theme), plus field_writes.py: what
...
1. Run **parity-review** and whatever review agents your environment provides, in parallel. **An agent that fails to load is a hard stop, not something to work around**. Fix the resolution first (usually: restart Claude Code from the repository root so project-local agents are discovered).
2. **Triage every finding against the frozen scope.** Nothing new enters it unless it is a correctness or security defect in code that change introduced. Every agent labels each finding as introduced by the diff or pre-existing, so this is a lookup rather than a judgement call. A real finding belonging to another stream is filed against that stream — it does not get fixed here, and it does not get dropped.
3. Address the findings that survive triage.
4. **Re-run every agent from step 1** — not just the ones that had findings. Repeat steps 2–4 until every agent returns APPROVE on the **same** tree. Never skip an agent because it approved on an earlier round — that approval is invalidated by every subsequent edit.
5. **Stop once the review converges.** When a round returns no criticals and no highs, and what remains is assertion strength, naming, or tidiness, that is the end of the loop: file what is relevant, cut what is not, say which is which, and commit. Two safeguards:
   - **Every cut is stated explicitly with its reason.**
   - **Every deferral gets a tracker issue naming the stream it lands in.**
6. Plan the atomic commit split before staging anything.
7. Run `command make check`. The review agents in step 1 are a workflow you run, not a make target.
