...
describe('useDataStore', () => {
  ...
  describe('file import flows', () => {
    it('loadImportSources fetches and caches the importable plugins', async () => {
      // Annotated, not inferred: the assertion below compares the cache to
      // this literal, so only the type makes a field the endpoint really
      // returns — `accepted_extensions`, which the modal builds its `accept`
      // filter and help text from — impossible to leave out.
    ...
    it('runImport posts a multipart body and starts sync polling', async () => {
      const importResult: ImportResultResponse = {
        ...
        // The plugin name the caller supplied, echoed back by the server —
        // NOT the internal "Import: <display name>" sync job label.
        source: 'csv_import',
        items_synced: 3,
        total_items: 3,
        errors: [],
