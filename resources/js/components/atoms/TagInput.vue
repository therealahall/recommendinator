...
watch(
  ...
  async () => {
    ...
    const buttons = chipList.value?.querySelectorAll<HTMLElement>(REMOVE)
    const next = buttons?.length ? buttons[Math.min(pending, buttons.length - 1)] : undefined
...
/* This is not a .btn, so the global disabled rule misses it and a locked × reads
   as a live one. */
