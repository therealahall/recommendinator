...
- **Component organization follows strict Atomic Design** (Brad Frost's methodology). This is not optional — it is how the component hierarchy is structured and enforced:
  ...
  **Atoms** (`components/atoms/`): The smallest UI building blocks. No business logic. No store access. Pure props in, events out. Examples: `BaseButton`, `BaseBadge`, `BaseSpinner`, `BaseInput`, `BaseSelect`, `BaseCard`, `EmptyState`. An atom renders a single visual element with styling variants via props. If you can't describe what it renders in 3 words ("a styled button"), it's not an atom.

  **Molecules** (`components/molecules/`): Simple, focused combinations of atoms that form a single functional unit. Minimal or no store access — prefer props/events. Examples: `StarRating` (atoms: clickable star spans + clear button), `ScorerSlider` (atoms: label + range input + value display), `FormGroup` (atoms: label + input/select), `SyncProgressBar` (atoms: progress bar + label). A molecule answers one user question ("what's the rating?" / "what weight?").
