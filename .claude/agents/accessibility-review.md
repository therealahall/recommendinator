...
| MEDIUM | Reduces usability without blocking | Decorative images without `alt=""`; missing `aria-expanded` on a disclosure |

Refinements to already-working markup are not findings — a more descriptive label, a tidier ARIA pattern, a missing `autocomplete` or `aria-current`. If that is all that is left, approve and say so.
...
### High Issues (Should Fix) / Medium (Consider)
